...
```{r data-import-merged, eval = FALSE}
bikes_raw <- import(here("bike_data", "all_bikes.rds"))
```

# Data Wrangling

```{r data-wrangling, eval = FALSE, echo = TRUE}
bikes <- bikes_raw %>% 
...
```{r plotting, eval = FALSE, echo = TRUE}
(sum_gg <- bike_sum2 %>% 
...
```{r include-plot, include = TRUE}
knitr::include_graphics(path = here("outputs", "comparison_plot.png"))
```
...
1.  A shorter time for Casual Riders before they have to pay an additional fee.
    This could incentivize frequent Casual Riders to upgrade to Member Riders.

2. A promotion for Casual Riders to have cheaper rates on the weekend.

3.  A different pricing strategy that incentivizes Casual Riders to convert to
    Member Riders if they do ride during the week.